package com.aj.datastructrure.list.linkedlist.circle;
 ...
 * @param <E>
 */
public class AJ_TwowayCircleLinkedList<E> extends AbstractList<E> {
    /**
     * 两个元素：
     * 1.链表的元素个数：size
     * 2.第一个节点 ： firstnode
    ...
    public E remove(Node<E> node) {
        if (size == 1) {
            ...
            prev.nextNode = next;
            next.prevNode = prev;
            if (node == firstNode) { // 相当于 index == 0
                firstNode = next;
            }
            if (node == lastNode) { // 相当于 index == size - 1
                lastNode = prev;
            }
        }
        size --;
